...
def browse_tracks(letter):
    ...
    query = "SELECT DISTINCT artist FROM tracks WHERE status = 1 AND (case when artist ilike 'The %' then substr(upper(artist), 5, 100) else upper(artist) end) >= '{letter}' LIMIT 20".format(cols=Track.columns, letter=letter)
    print(query)
    with _conn, _conn.cursor() as cur:
        cur.execute(query)
