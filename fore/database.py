...
def browse_tracks(letter):
    ...
    query = "SELECT DISTINCT artist FROM tracks WHERE status = 1 AND (case when artist ilike 'The %' then substr(artist, 5, 100) else artist end) >= '{letter}'".format(cols=Track.columns, letter=letter)
    print(query)
    with _conn, _conn.cursor() as cur:
        cur.execute(query)
